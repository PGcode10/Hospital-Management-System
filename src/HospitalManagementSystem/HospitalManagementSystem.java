...
public static void bookAppointment(Patient patient, Doctor doctor, Connection connection, Scanner scanner) {
        ...
        if (patient.getPatientById(patientId) && doctor.getDoctorById(doctorId)) {
            ...
            System.out.println("Either Doctore or Patient does not exist.");
            return;
            
        }

        // Here you would typically check if the doctor exists and then proceed with booking logic
    ...
    public static boolean checkDoctorAvailability(Connection connection, int doctorId, String appointmentDate) {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                if (count == 0) {
                    return true; // Doctor is available if no appointments on that date
                    
                } else {
                    return false; // Doctor is not available if there are appointments on that date
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
